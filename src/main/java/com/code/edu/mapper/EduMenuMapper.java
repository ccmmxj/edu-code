...
public interface EduMenuMapper extends BaseMapper<EduMenu> {
    @Select("select em.* from edu_role_menu_related ermr join edu_role er on ermr.role_id = er.id and er.is_deleted = 0 join edu_menu em on em.id = ermr.menu_id and em.is_deleted = 0 join edu_role_user_related erur on erur.role_id = er.id and erur.is_deleted = 0 join edu_user eu on eu.id = erur.user_id and eu.is_deleted = 0 where eu.id = #{userId} and em.parent_id = 0 and ermr.is_deleted = 0")
